...
mod tests {
    ...
    fn to_degrees_from_each_unit() {
        assert_eq!(Angle::deg(45.0).to_degrees(), 45.0);
        assert_eq!(Angle::rad(std::f32::consts::PI).to_degrees(), 180.0);
        assert_eq!(Angle::grad(100.0).to_degrees(), 90.0);
        assert_eq!(Angle::turn(0.5).to_degrees(), 180.0);
    }

    #[test]
    fn to_radians_from_each_unit() {
        assert!((Angle::deg(180.0).to_radians() - std::f32::consts::PI).abs() < 1e-5);
        assert_eq!(Angle::rad(2.0).to_radians(), 2.0);
        assert!((Angle::grad(200.0).to_radians() - std::f32::consts::PI).abs() < 1e-5);
        assert!((Angle::turn(0.5).to_radians() - std::f32::consts::PI).abs() < 1e-5);
    }
