...
impl fmt::Display for AtRule {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AtRule::Container { name, query, rules } => container::render(f, name.as_ref(), query, rules),
            AtRule::Scope { root, limit, rules } => scope::render(f, root.as_ref(), limit.as_ref(), rules),
            AtRule::Layer { name, rules } => layer::render(f, name.as_ref(), rules),
            ...
            AtRule::Page { pseudo, declarations } => page::render(f, pseudo.as_ref(), declarations),
            AtRule::Import { url, supports, media } => import::render(f, url, supports.as_ref(), media.as_ref()),
            AtRule::Charset { encoding } => charset::render(f, encoding),
            AtRule::Namespace { prefix, url } => namespace::render(f, prefix.as_ref(), url),
        }
    }
}
