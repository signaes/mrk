...
impl Color {
    ...
    /// channels.
    pub fn color(space: impl Into<Ident>, channels: Vec<f32>) -> Self {
        Color {
...
fn gamut_map_srgb(r: f32, g: f32, b: f32, max_iters: u32) -> (f32, f32, f32) {
    ...
    for _ in 0..max_iters {
        ...
        let in_gamut = r2 >= -1e-6 && r2 <= 1.0 + 1e-6
            && g2 >= -1e-6 && g2 <= 1.0 + 1e-6
            && b2 >= -1e-6 && b2 <= 1.0 + 1e-6;
        if in_gamut {
            lo = mid;
...
impl Default for ColorMixSpace {
    fn default() -> Self {
        ColorMixSpace::Oklch
    }
}
...
fn split_outside_parens<'a>(input: &'a str, separator: u8) -> Vec<&'a str> {
...
fn parse_color_mix(args: &str) -> Result<Color, ColorParseError> {
    ...
    c.skip_ws();
    let hue_method = if !c.is_done() && c.peek().map_or(false, |b| b.is_ascii_alphabetic()) {
...
mod tests {
    ...
    #[test]
    fn color_function() {
        let c = Color::color("display-p3", vec![1.0, 0.0, 0.0]);
