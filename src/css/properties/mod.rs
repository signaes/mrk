...
pub(crate) fn parse_value_list(s: &str) -> Value {
    ...
    let values: Vec<Value> = parts.into_iter().map(parse_value).collect();
    Value::List(values)
}
...
fn unit_to_value(n: f32, unit: &str) -> Value {
    match unit {
        ...
        // Percentage
        "%" => Value::Percentage(Percentage::new(n)),
        // Angle
...
fn parse_function_value(name: &str, args_str: &str) -> Value {
    // Function arguments are comma-separated (CSS spec).
    let arg_vec: Vec<Value> = if args_str.trim().is_empty() {
        Vec::new()
    } else {
        args_str.split(',').map(|a| parse_value(a.trim())).collect()
    };
...
fn parse_var_value(args_str: &str) -> Value {
    ...
    if let Some(cp) = cp {
        if fallback_str.is_empty() {
            ...
            let fallback = parse_value(fallback_str);
            Value::Function {
                name: Cow::Borrowed("var"),
